...
public class HTTPDHelpModule extends HTTPDModule {
    ...
    public String getBody() {
        ...
        if ((commandMap = CommandLoader.getCommandMap()) == null
                || (knownCommands = CommandLoader.getKnownCommands(commandMap)) == null) {
        ...
        while (itCommandsByPlugin.hasNext())
        {
            ...
            Collections.sort(commands, new Comparator<Command>()
            {
                ...
                {
                    if (a instanceof CommandLoader.TFM_DynamicCommand && b instanceof CommandLoader.TFM_DynamicCommand) {
                        String aName = ((CommandLoader.TFM_DynamicCommand) a).getCommandInfo().getLevel().name() + a.getName();
                        String bName = ((CommandLoader.TFM_DynamicCommand) b).getCommandInfo().getLevel().name() + b.getName();
            ...
            for (Command command : commands)
            {
                ...
                {
                    AdminLevel tfmCommandLevel = ((CommandLoader.TFM_DynamicCommand) command).getCommandInfo().getLevel();
